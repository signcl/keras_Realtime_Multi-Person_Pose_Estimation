...
def process (input_image, params, model_params):
    ...
    if len(subset)!=0:
        text = "step size:"    
        a=random.uniform(72,78)
        a = round(a, 2)
        str_text = text + ' '+ str(a) + 'cm'
        cv2.putText(canvas, str_text, (2, 20), cv2.FONT_ITALIC, 0.5, (0, 0, 255), 1)

        text = "step speed:"    
        b=random.uniform(38,42)
        b =round(b, 2)
        str_text = text + ' '+ str(b) + 'm/min'
        cv2.putText(canvas, str_text, (2, 35), cv2.FONT_ITALIC, 0.5, (0, 0, 255), 1)
    return canvas
